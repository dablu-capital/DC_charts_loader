...
class FileSelector:
    ...
    def get_available_files(self) -> List[str]:
        ...
        try:
            ...
            main_files = [
                ...
                for f in all_files
                if not (
                    f.name.endswith("_data.feather")
                    or f.name.endswith("_min_data.feather")
                )
            ]
    ...
    def display_file_menu(self, files: List[str], default: Optional[str] = None) -> str:
        """
        ...
        while True:
            ...
            try:
                ...
                if 1 <= choice_num <= len(files):
                    ...
                    print(
                        f"❌ Invalid selection. Please enter a number between 1 and {len(files)}"
                    )
