...
class TestChartsDailyData:
    ...
    def test_init(self, mock_load_daily_df):
        ...
        mock_data_df = pd.DataFrame(
            {"ticker": ["AAPL"], "date": [datetime(2023, 1, 1)], "close": [100.0]}
        )

        mock_load_daily_df.side_effect = [mock_charts_df, mock_data_df]

        with patch(
            "src.models.apply_indicators", return_value=mock_data_df
        ) as mock_apply:
...
class TestChartsMinuteData:
    ...
    @patch("src.models.load_daily_df")
    def test_init(self, mock_load_daily_df, mock_apply_indicators, sample_config_data):
        ...
        dict_folder = sample_config_data["general"]["data_path"]
        dict_filename = sample_config_data["general"]["data_filename"]
        dict_path = f"{dict_folder}/{dict_filename}"
        data_path = dict_path.replace(".feather", "_min_data.feather")

        charts_data = ChartsMinuteData(dict_path, data_path)

        # Verify initialization
        assert charts_data.current_index == 0
        assert charts_data.current_timeframe == "1m"
        assert charts_data.dict_filename == "./data/default.feather"
        assert charts_data.data_filename == "./data/default_min_data.feather"

        # Verify data loading was called
        assert mock_load_daily_df.call_count == 1

    @patch("src.models.load_daily_df")
    @patch("src.models.apply_indicators")
    def test_set_timeframe(
        self, mock_apply_indicators, mock_load_daily_df, sample_config_data
    ):
        """Test setting timeframe."""
        # Mock the data loading
        mock_charts_df = pd.DataFrame(
            {"ticker": ["AAPL"], "date": [datetime(2023, 1, 15)], "volume": [1000]}
        )
        mock_data_df = pd.DataFrame(
            {"ticker": ["AAPL"], "date": [datetime(2023, 1, 15)], "close": [150.0]}
        )
