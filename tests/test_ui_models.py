...
class TestChartPlotter:
    ...
    def test_init(self):
        """Test ChartPlotter initialization."""
        chart_data = Mock()
        chart = Mock()
        
        plotter = ChartPlotter(chart_data, chart)
        
        assert plotter.chart_data is chart_data
        assert plotter.chart is chart
        assert plotter.drawing_ids == []

    def test_init_without_chart(self):
        """Test ChartPlotter initialization without chart."""
        chart_data = Mock()
        
        with patch('src.ui.models.Chart') as mock_chart_class:
            mock_chart = Mock()
            mock_chart_class.return_value = mock_chart
            
            plotter = ChartPlotter(chart_data)
...
class TestDualChartPlotter:
    ...
    @patch('src.ui.models.plot_indicators')
    @patch('src.ui.models.plot_chart')
    def test_setup(self, mock_plot_chart, mock_plot_indicators, mock_on_maximize, mock_on_timeframe_change):
